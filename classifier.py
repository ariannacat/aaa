...
class Preprocessor:
    ...
    def __call__(self, adata: an.AnnData, batch_key: Optional[str] = None) -> Dict:
        """
    ...
    def check_logged(self, adata: an.AnnData, obs_key: Optional[str] = None) -> bool:
        """
...
'''Bins Tuning'''
'''Elastic Net'''
def tune_binning_enet(adata):

    MSE_enet = []

    tuning = pd.DataFrame()
    tuning.index = range(10,1001,10)

    for bins in range(10,1001,10):
        ...
        df = pd.DataFrame(adata.layers["X_binned"])
        df.index = adata.obs_names
        df.columns = adata.var_names
        df.insert(0, 'Condition', adata.obs["Condition"])

        X = df.iloc[:,1:].astype(int)
        y = df["Condition"].astype(int)
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
        
        elastic_net = SGDClassifier(loss="log_loss", penalty="elasticnet")
        enet_fit = elastic_net.fit(X_train, y_train)
        y_pred = enet_fit.predict(X_test)
        mse_enet = mean_squared_error(y_test,y_pred)
        MSE_enet.append(mse_enet)

    tuning["MSE"] = MSE_enet
    
    plt.figure(figsize=(10, 6))
    plt.plot(tuning["MSE"], marker='o')
    ...
    min_index = tuning['MSE'].idxmin()
    print("The best number of bins for Elastic Net is", min_index)

    return min_index

'''XGBoost'''
def tune_binning_xgb(adata):
    ...
    for bins in range(10,1001,10):
        ...
        xgb_fit = xg_boost.fit(X_train, y_train)
    ...
    min_index = tuning['MSE'].idxmin()
    print("The best number of bins for XGBoost is", min_index)

    return min_index

'''Tuning SDG Classifier'''
def tune_sdg_classifier(X, y, n_trials=100, patience=20, n_splits=10):
    ...
    for i in range(n_splits):
        ...
        for _ in range(n_trials):
            ...
            if study.best_trial == best_trial:
                unchanged_counter += 1
                if unchanged_counter >= patience:
                    print(f"Best trial unchanged for {patience} consecutive trials. Stopping optimization.")
                    break
            else:
        ...
        best_clf = SGDClassifier(**best_params)
    ...
    print("\nOverall Results after 10 iterations:")
    print(f"Maximum Accuracy: {max_accuracy}")
    print(f"Average Accuracy: {avg_accuracy}")
    print(f"Average Mean Squared Error: {avg_mse}")
    print(f"Average ROC AUC Score: {avg_roc}")
    
    plt.figure(figsize=(10, 6))
    plt.plot(all_accuracies, marker='o')
    plt.axhline(y = avg_accuracy, color='r', linestyle='--', label=f'Average Score: {avg_accuracy:.4f}')
    plt.title(f'Accuracy per Trial')
    plt.xlabel('Trial')
    plt.ylabel('Accuracy')
    plt.grid(True)
    plt.show()
    return max_accuracy, avg_accuracy, avg_mse, avg_roc

'''Tuning XGBoost'''
def tune_xgb_classifier(X,y, n_trials = 100, patience = 20, n_splits = 10):
